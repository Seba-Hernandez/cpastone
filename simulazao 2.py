...
class CallCenter:
    ...
    def manejo_listas_eventos(self, string_evento):
        ...
        if string_evento == "retornar_evento_termino":
                ...
                print("Lista Colas",len(self.lista_eventos_cola))
                ...
                #         evento_lista.tiempo_en_cola = self.tiempo_actual - evento_lista.tiempo_llegada
                #         evento_lista.actualizar_duracion_total()
                ...
                if min_llegada <= min_termino:
                    ...
                    #     evento_lista.actualizar_duracion_total()
                    #     self.tiempo_actual = evento_lista.tiempo_llegada + evento_lista.duracion_total
                    #     evento_lista.tiempo_llegada = self.tiempo_actual
                    #     tiene_ambulancia = self.cargar_evento_llegada(evento_lista)
                    #     if tiene_ambulancia == True:
                    #         self.lista_de_terminos.append(evento_lista)
    ...
    def cargar_evento_llegada(self, evento):
        ...
        for lista in lista_rutas_bases:
            ...
            for objeto_base in self.lista_bases_creadas:
                if (objeto_base.nodo_asociado == id_base) and (objeto_base.activada == True):

                    for ambulancia in objeto_base.lista_ambulancias_base:

                        #print(ambulancia.disponible)
                        if ambulancia.disponible == True:
                            ...
                            hora_salida_centro = floor(evento.hora_partida + tiempo_base_evento + evento.atencion)
                            hora_salida_centro_2 = evento.hora_partida + tiempo_base_evento + evento.atencion
        ...
        if hay_ambulancia == 0:
            ...
            lista_centros_bases = self.dicc_centros_bases[fstring2][str(id_centro)]
            # print(evento._id, "EVENTO IDD")
            for base in lista_centros_bases:
                if base[2] == id_nodo_base_retornar:
                    tiempo_centro_base = base[0]
