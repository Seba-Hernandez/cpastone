...
def rutas_tiempo_minimo_dijkstra_bidireccional(Grafo,lista_origenes,lista_destinos, string_tiempo):
    ...
    diccionario_rutas={}
    contador=1
    diccionario_rutas_tiempo_minimo_eventos_a_centros = dict()
    for origen in lista_origenes:
        evento = origen
        diccionario_rutas_tiempo_minimo_eventos_a_centros[evento] = []
    for origen in lista_origenes:
        for destino in lista_destinos:
            ...
            print("")
            contador+=1
            lista_de_3_elementos_sobre_ruta_evento_a_centro = [tiempo_ruta_tiempo_minimo, lista_ruta_tiempo_minimo, id_centro]
            diccionario_rutas_tiempo_minimo_eventos_a_centros[id_evento].append(lista_de_3_elementos_sobre_ruta_evento_a_centro)
    B=time.time()
    segundos=B-A
    minutos=segundos/60
    horas=minutos/60
    for origen in lista_origenes:
        evento = origen
        diccionario_rutas_tiempo_minimo_eventos_a_centros[evento].sort()
        ruta_mas_cercana_evento_a_centro = diccionario_rutas_tiempo_minimo_eventos_a_centros[evento][0]
        diccionario_rutas_tiempo_minimo_eventos_a_centros[evento] = ruta_mas_cercana_evento_a_centro

    return diccionario_rutas_tiempo_minimo_eventos_a_centros
...
lista_reducida_eventos = lista_eventos[1:len(lista_eventos)]

#5 días
#lista_reducida_eventos = lista_eventos[1:307]
...
print(f"Los {len(lista_nodos_eventos)} eventos del de todo el año se asocian a {len(set(lista_nodos_eventos))} nodos diferentes, por lo tanto existen {len(lista_nodos_eventos) - len(set(lista_nodos_eventos))} pares de eventos que se asocian a un mismo nodo de la ciudad")
...
print("Existen 1 par de centros de salud asociados a un mismo nodo.")
...
print(f"")
print()
print(f"Largo de lista con id de nodos asociados a centros de salud y con id de nodos asociados a eventos: {len(set(lista_nodos_eventos))+len(set(lista_centros_destinos))}")
print(f"Largo de set con id de nodos asociados a centros de salud y con id de nodos asociados a eventos: {len(set(lista_nodos_eventos+lista_centros_destinos))}")
print(f"Existen {len(set(lista_nodos_eventos))+len(set(lista_centros_destinos))- len(set(lista_nodos_eventos+lista_centros_destinos))} ocasiones en que un mismo nodo está asociado tanto a un evento como a un centro de salud.")
...
contador = 0
for i in range(24):
    lista_eventos_origenes = list(set(id_nodos))
    lista_centros_destinos = list(set(lista_centros_destinos))
    fstring = f"Tiempo {i}"
    diccionario_dijkstra_bidireccional_tiempo_eventos_a_centros = rutas_tiempo_minimo_dijkstra_bidireccional(Grafo, lista_eventos_origenes, lista_centros_destinos, fstring)
    diccionario_24_tiempos[fstring] = diccionario_dijkstra_bidireccional_tiempo_eventos_a_centros
    cantidad_de_rutas_por_tiempo = len(lista_eventos_origenes)*len(lista_centros_destinos)
    contador += cantidad_de_rutas_por_tiempo
B = time.time()

print(f"El tiempo que tomó calcular las {contador} = {len(lista_eventos_origenes)}*{len(lista_centros_destinos)}*24 rutas a tiempo mínimo fue de {B-A} segundos, equivalente a {(B-A)/60} minutos, equivalente a {(B-A)/3600} horas")
...
# print(diccionario_24_tiempos["Tiempo 0"][3316])
...
# print(diccionario_24_tiempos["Tiempo 0"][3316][1])
# print(diccionario_24_tiempos["Tiempo 0"][3316][2])



diccionario_24_tiempos_rutas_eventos_a_centros = diccionario_24_tiempos
#diccionario_24_tiempos_rutas_eventos_a_centros = {"Tiempo 0": {id_evento1: [tiempo_ruta_minima_a_centro_mas_cercano, [ruta_minima_a_centro_mas_cercano], id_centro_mas_cercano], ..., id_evento79: [tiempo_ruta_minima_a_centro_mas_cercano, [ruta_minima_a_centro_mas_cercano], id_centro_mas_cercano]}, ..., "Tiempo 23": {id_evento1: [tiempo_ruta_minima_a_centro_mas_cercano, [ruta_minima_a_centro_mas_cercano], id_centro_mas_cercano], ..., id_evento79: [tiempo_ruta_minima_a_centro_mas_cercano, [ruta_minima_a_centro_mas_cercano], id_centro_mas_cercano]}}
#A LA SIMULACIÓN HAY QUE PASARLE EL DICCIONARIO diccionario_24_tiempos_rutas_bases_a_eventos

with open("dijkstra_final_eventos_a_centros.json", "w") as f:
   json.dump(diccionario_24_tiempos_rutas_eventos_a_centros, f)
