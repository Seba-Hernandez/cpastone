...
print(f"Los {len(lista_nodos_eventos)} eventos del año, se asocian a {len(set(lista_nodos_eventos))} nodos diferentes, por lo tanto existen {len(lista_nodos_eventos) - len(set(lista_nodos_eventos))} pares de eventos que se asocian a un mismo nodo de la ciudad")
...
print(f"Existen {len(set(lista_nodos_eventos))+len(set(lista_centros_destinos)) - len(set(list(set(lista_nodos_eventos))+list(set(lista_centros_destinos))))} ocasiones que un mismo nodo está asociado tanto a un evento como a un centro de salud.")
...
#Código para sacar rutas a tiempo mínimo usando dijkstra bidireccional con origen los nodos asociados a los eventos de eventos.csv y destino los centros de salud
A = time.time()
contador = 0
...
with open("dijkstra_final_centros_a_bases.json", "w") as f:
   json.dump(diccionario_24_tiempos_rutas_centros_a_bases, f)